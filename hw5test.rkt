...
(define tests
  (test-suite
   ...
   (check-equal? (void? (check-exn #rx"MUPL call applied to non-closure" (lambda () (eval-exp (call (int 7) (int 7)))))) #t "2-17: bad call")
   ...
   ;; There is an autograder and I'm tight on time so I will write less test cases from this point forward
   ...
   (check-equal? (eval-exp (call (call mupl-map (fun #f "x" (add (var "x") (int 7)))) (apair (int 1) (aunit)))) 
                 (apair (int 8) (aunit)) "4a-1: 1 element")
   (check-equal? (eval-exp (call (call mupl-map (fun #f "x" (add (var "x") (int 7)))) (apair (int 1) (apair (int 4) (aunit))))) 
